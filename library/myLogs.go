...
func NewMyLogs(rootpath, dir, filename, logext string, httpreq *http.Request) (myLog *MyLogs, err error) { //, maxlogfilenum int, maxlogfilesize int
	if rootpath == "" {
	...
	if myLog.Filename == "" {
		...
		myLog.Filepath = rootpath + myLog.GetSeparator() + dir + myLog.GetSeparator()
	}
	err = myLog.start()
	return
}
